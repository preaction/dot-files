...
if initialize_session "hannibal-www"; then
  ...
  tmux send-keys -t "$session:$window.2" C-z "cd $session_root; redis-server" Enter
  tmux send-keys -t "$session:$window.3" C-z "cd $session_root/../hannibal-cia; carton exec morbo script/cia-public -l http://*:3000" Enter
  tmux send-keys -t "$session:$window.4" C-z "cd $session_root; carton exec morbo script/hannibal-www -l http://*:3001" Enter
