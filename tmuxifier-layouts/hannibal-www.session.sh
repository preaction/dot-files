...
if initialize_session "hannibal-www"; then
  ...
  tmux send-keys -t "$session:$window.2" C-z "cd $session_root/../hannibal-cia; carton exec morbo script/hannibal -l http://*:3001" Enter
  tmux send-keys -t "$session:$window.3" C-z "cd $session_root; carton exec morbo script/hannibal-www" Enter
